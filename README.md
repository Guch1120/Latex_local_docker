...
以前に別のLaTeXエディタ用コンテナを起動していると、`localhost:8000` が別ディレクトリの環境を指している場合があります。
その場合は起動中のコンテナを確認してください。

```bash
docker ps
```

このディレクトリの環境を起動し直す場合は、以下を実行します。
...
- [ ] RSJ,SI論文テンプレート対応 <br>
- [x] 文字検索し置換 <br>
- [x] 表示しているoutlineをクリックしてセクションに飛ぶ <br>
- [x] 表示logのコピペ <br>
- [x] エディタ背景がもやっとしているのが不快 <br>
- [x] 画面分割してもエディタレイアウトが崩れないこと <br>
- [ ] テンプレートプロファイルの選択 <br>
- [x] ファイルタブを閉じるようにする<br>
- [x] セーブとコンパイルのボタン配色 <br>
- [x] コンパイル中とコンパイルボタンの色
- [ ] Windows向けの簡単起動スクリプト対応（Docker Desktop確認、起動、ブラウザ表示） <br>
- [ ] Windows向けの停止スクリプト対応 <br>
